...
3. $$p$$值是在假设$$H_0$$即原假设为真的条件下，出现数据$$D$$的可能性，即为$$P(D|H_0)$$,但是我们实际想要得到的是$$P(H_0|D)$%,即在已有数据$$D$$的情况下判断$$H_0$$出现的可能性。$$P(H_0|D)=\frac{P(D|H_0)\times P(H_0)}{P(D)}$$,因此当$$P(H_0)$$过大或者$$P(D)$$值过小时，都会影响实际$$P(H_0|D)$$，从而导致$$p$$值得预测不准确。
